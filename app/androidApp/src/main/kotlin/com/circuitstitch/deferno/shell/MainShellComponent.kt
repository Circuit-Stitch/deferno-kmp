 ...
 * the Tasks Destination; a Tasks "add to plan" bubbles up via [Output] for the host to mirror) — the
 * same routing the throwaway demo host owned, now part of the real shell.
 */
interface MainShellComponent {
    ...
    /**
     * Route Android system-back within the Main shell: first let the active Destination dismiss its
     * own tier-2/tier-3 state, then fall back from a non-home Destination to the [Destination.Plan]
     * home. Returns whether back was consumed (`false` → the host lets the platform exit the scene).
     */
    fun onBack(): Boolean
