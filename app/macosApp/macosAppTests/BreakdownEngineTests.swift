...
/// LLM-free coverage of the Breakdown state machine (Deferno#525) — the macOS port of
/// `iosAppTests/BreakdownEngineTests.swift`, landed with the engine itself in #368 Tranche 5d.
///
/// **Why a second copy of the suite and not a shared one:** #368 §6 resolved the Breakdown route as (a) —
/// port the Swift rather than drive the shared Kotlin `BreakdownEngine` — which means the repo now carries
/// the same state machine three times (Kotlin in `app/shell`, Swift in `iosApp`, Swift here). That is a real
/// cost, and the mitigation is that each copy is pinned by its own suite: a behaviour change made in one and
/// not the others fails here rather than drifting silently onto the Mac. Landing the engine without this file
/// would have shipped 220 lines of decision logic into `macosAppTests` — a target CI runs — with zero
/// coverage. If the three engines are ever unified, delete this file *with* its twin, not before.
///
/// Per the PRD's testing decisions we assert **external behavior** — which moves run with what args, given a
/// scripted sequence of classified answers, and the terminal state — never internals. A `ScriptedClassifier`
/// stands in for Apple Intelligence and a `SpyMoves` records the offline-first moves, so the whole flow runs
/// with no model, on any Mac, including one with no Apple Intelligence at all.
///
/// Deliberately **no `import Deferno`** here, unlike the sibling `DefernoThemeTests`. The Kotlin framework
/// exports its own `ImpedimentClassifier` (the shared `BreakdownComponent.bind` seam), so a file importing
/// both modules would make that name — and `BreakdownEngine` — ambiguous, since neither module is local to
/// the test module. Nothing in this suite needs a Kotlin type, so the narrower import is also the correct one.
@MainActor
