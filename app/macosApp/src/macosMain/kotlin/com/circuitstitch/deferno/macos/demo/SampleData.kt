...
/**
 * In-memory sample content for the **iOS demo harness** (#51) — the SwiftUI counterpart of the
 * Android shell's `demo/SampleData` (#27/#55). The Tasks + Plan SwiftUI Views render this through the
 * [DemoTaskRepository] / [DemoPlanRepository] fakes so the screens are runnable on the simulator
 * *before* the real iOS app shell + DI exist. Deliberately small and calm (design-principles.md): a
 * couple of active items, one parent that decomposes into steps, and a short daily plan. The real app
 * will read from DI-provided repositories (a follow-up, #68/ADR-0014); this stays a scaffold fixture.
 */
internal object SampleData {
    ...
    private val planTaskIds: List<TaskId> = listOf(TaskId("t-1"), TaskId("t-2"), TaskId("t-3"))

    /** The plan's Tasks resolved from [tasks], in plan order — seeds the [DemoPlanRepository]. */
    val planTasks: List<Task> = planTaskIds.mapNotNull { id -> tasks.firstOrNull { it.id == id } }
