...
///
/// The macOS twin of iosApp's `Breakdown/BreakdownTypes.swift`, ported byte-for-byte in #368 G10: the file
/// names no platform API and no Kotlin type, so a verbatim copy is the *correct* port, not a lazy one. It
/// deliberately does **not** `import Deferno` — staying framework-free is what keeps the exported Kotlin
/// `Deferno.Task` out of scope here and leaves the engine testable without a DI graph. (The Kotlin
/// classifier types land in Swift under an `Agent` prefix — `AgentImpedimentClassifier` etc. — so none of
/// the names below collide with the framework.)
