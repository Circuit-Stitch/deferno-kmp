...
/// The deterministic **Breakdown** state machine (Deferno#525), native to macOS. It runs the impediment
...
/// `_Concurrency.Task { … }` (bare `Task` resolves to the exported Kotlin `Deferno.Task`) and tests just
/// `await` them — every transition is deterministic.
///
/// **Why a second engine.** A shared Kotlin `BreakdownEngine` also exists (`app/shell`, driving the Compose
/// surfaces) and is exported into `Deferno.framework` — so macOS *could* have driven it through
/// `BreakdownComponent.bind()/submit()/confirmDrop()` instead of carrying this Swift twin. #368 §6 left the
/// route open; it is **decided here as "port the Swift twin", deliberately**, for two reasons: (1) the
/// on-device classifier (`FoundationModelsClassifier`) was already written dual-platform —
/// `@available(iOS 26.0, macOS 26.0, *)` on every declaration — and names no Kotlin type, so the Swift path
/// costs a file copy and nothing else; (2) driving the Kotlin engine from macOS would mean deleting iOS's
/// tested Swift engine as collateral inside a *macOS parity* PR. Deduping the two engines is a separate,
/// cross-surface refactor. **This fork is chosen, not missed** — the two files are byte-identical twins, so
/// a change to one belongs in both until that refactor lands.
@MainActor
