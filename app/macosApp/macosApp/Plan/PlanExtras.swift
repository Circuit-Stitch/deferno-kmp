...
// The Plan's two **local** decision surfaces (#368 G16 — the macOS twins of `PlanScreen.kt`'s WhatsNext +
// Focus sub-screens, ported from the iOS `PlanExtras.swift` under ADR-0028's blessing of near-verbatim
// per-platform View bodies). Both are derived purely from the already-loaded `[Task]`: the shared
// `PlanState` is unchanged — it carries no what's-next/focus state — so there is no Decompose child, no
// bridge seam and no shell ripple. `PlanView` presents them as sheets; see the note there for why Focus
// is a sheet rather than an ADR-0033 auxiliary window in this pass. Calm, parchment, mono-eyebrow voice.
...
/// The breathing focus ring around a clock glyph. Static when `reduceMotion` — this app takes reduced
/// motion literally rather than merely shortening the animation (the same reason `LoadingStrip` is a calm
/// static strip and not a spinner); otherwise a slow 2.6s scale + alpha pulse. The ring is pure decoration
/// beside the task title, so it is hidden from VoiceOver outright.
