...
// The signature "See the trees" connected-tree filigree (macOS twin of the iOS `TreeFiligree.swift` /
// feature/tasks/ui `TreeAtoms.kt`, #237 / ADR-0028 per-platform duplication): a curvy rail of per-depth
// vertical spines + a rounded elbow that lands in the kind node, and a node that is the kind dot (leaf) or
// an accent fold-disc with a rotating chevron (parent). Drawn off the bridged `ItemRow` geometry
// (spine / depth / hasChildren / isExpanded) — no shared-component change.
...
enum TreeGeometry {
    ...
    /// region ~1px left — which would drag the root's disc + descend line off the trunk its children draw.
    /// Clamping to the node width keeps every depth's column on the same x.
        ...
        Canvas { ctx, size in
            ...
            for i in 0..<depth {
                ...
                if i < depth - 1 {
                    ...
                    // path at that corner. Mirrors the Compose twin (TreeAtoms.treeRail).
                    let r: CGFloat = 8
                    let corner = midY - r
