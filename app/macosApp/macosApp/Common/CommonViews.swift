...
/// de-emphasized + struck through. Stateless: the handlers take their params from the row, never from
/// observed state.
struct ItemRowView: View {
    ...
    /// muted mono filigree — it *looks* decorative, so a VoiceOver user stepping through the row would skip
    /// the one thing that says this item comes back. The `MonoMeta` hides itself so it is not said twice.
        ...
        return HStack(spacing: 0) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                Text(row.item.title)
                    ...
                    // swallow both controls. `blocked`/`Blocker` stay on their own badges, which already
                    // label themselves.
                ...
                if let recurrence {
                    MonoMeta(recurrence.text)
                        .padding(.top, 2)
                        // Already spoken as part of the title's label, so it stays out of the focus order.
