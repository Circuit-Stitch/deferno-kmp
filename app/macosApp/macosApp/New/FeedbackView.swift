...
/// (`ShellBridgeKt.feedbackAddAttachment`) — the macOS twin of Android's SAF + `ContentResolver` read.
    ...
    private func categoryPicker(_ value: FeedbackState) -> some View {
        HStack(spacing: 8) {
            ForEach(ShellBridgeKt.feedbackCategories().indices, id: \.self) { i in
                ...
                SelectableChip(
                    label: ShellBridgeKt.feedbackCategoryLabel(category: category),
                    selected: ShellBridgeKt.feedbackCategoriesEqual(a: value.category, b: category)
