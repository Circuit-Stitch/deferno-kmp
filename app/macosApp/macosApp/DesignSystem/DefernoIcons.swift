...
/// The Deferno glyph set (macOS twin of core/designsystem `DefernoIcons.kt` and the iOS `DefernoIcon`,
/// #237/ADR-0028 per-platform duplication). The Compose side hand-builds brand `ImageVector`s; on the
/// Apple targets we map to **SF Symbols** — the native idiom that scales with Dynamic Type and matches
/// the platform — accepting minor drift on the two bespoke glyphs (Sparkle → `sparkles`, Play → `play.fill`).
enum DefernoIcon {
