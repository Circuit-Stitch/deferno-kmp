...
/// this just returns the model's JSON text (or a non-PII error category), gated on model availability.
///
/// FoundationModels is macOS 26+; the app deploys to 14.0, so every use is `@available`-guarded and the
/// framework is weak-linked (`-weak_framework FoundationModels` in project.yml). On an older Mac — or one
/// where Apple Intelligence isn't enabled/downloaded — `isAvailable()` is `false` and the seam answers
...
final class MacInference: NativeInference {
    ...
    func infer(
        ...
        if #available(macOS 26, *) {
            ...
            // No `@Generable` Swift type — the schema is a *runtime* contract Kotlin owns. So steer the
            // model with Kotlin's by-example shape (exact keys + types) and let Kotlin decode/validate
            // the reply. The shape closes the gap a name-only prompt leaves on a small on-device model.
            let steered = """
            \(instructions)
    ...
    /// A **non-PII** category for a generation failure — never the model output (ADR-0027). Kept coarse
    /// on purpose: it's a developer diagnostic, and the exact `GenerationError` case spellings aren't
    /// worth coupling to. ponytail: widen the mapping if a specific failure needs distinct UX.
    private static func category(of error: Error) -> String {
        #if canImport(FoundationModels)
        if #available(macOS 26, *), error is LanguageModelSession.GenerationError {
