 ...
 * the caller: this payload is also transported inside `core:domain`'s `ConvertItem` command, where a
 * pre-set stamp would be a merge key minted long before (or instead of) the request that carries it.
 * Null is dropped by [com.circuitstitch.deferno.core.network.DefernoJson], so an unstamped convert sends
