 ...
 *
 * - The **online-only create gate** (ADR-0016, #71) probes [isOnline] before POSTing. Create is the
 *   one write that is *not* offline-first: the server has no idempotency key in v0.1, so a queued
 *   create replayed after a network blip would duplicate the item. When offline the create writer
 *   refuses with a gentle "reconnect to save" — enqueuing nothing.
...
interface Connectivity {
     ...
     * is what impls must avoid, since it would suppress flushes and block creates while reachable.
     */
    val online: StateFlow<Boolean>

    /** Snapshot probe for the create gate (ADR-0016): the current [online] value by default. */
 ...
 * flush pass is ever skipped (the periodic tick + per-entry backoff govern alone), and the create
