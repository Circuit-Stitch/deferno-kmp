# Client-side agent: Koog in the shared core, premium inference behind a thin Deferno relay, propose-only v1
...
- **Inference is tiered: local preferred when available, off-device only by explicit opt-in, never
  silent.** Launch sequencing inverts the end-state on purpose: **v1 ships premium-only** (still
  opt-in) to exercise the full pipeline; local engines fast-follow (desktop Ollama, Android LiteRT
  when its Koog module matures; iOS deferred with its app). Engine choice and the opt-in are **App
  settings** (device-local); the premium entitlement is per-Account.
- **Premium inference is a Deferno-owned paid service behind a thin relay** — the OSS constraint
  forces the key behind a Deferno-operated endpoint. The relay is an **Anthropic-format passthrough**:
  PAT auth → entitlement check → usage metering → verbatim forward. The client points Koog's stock
...
**Considered and rejected.** *BYO user API key* (product decision: premium service instead — and it
remains additive later as another engine-catalog entry). *Backend agent endpoints* (duplicates agent
...
**Consequences.** The backend repo (`Kyle-Falconer/Deferno`) gains three prerequisites to file: the
relay endpoint, premium entitlement exposure, and usage metering. The accept paths need **no new
...
engine interface. Telemetry/metering of inference happens server-side at the relay, preserving
ADR-0009's no-client-analytics stance.
